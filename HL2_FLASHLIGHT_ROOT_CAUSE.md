...
```text
game\hl2\shaders\fxc\worldtwotextureblend_ps20b.vcs
```

## Installing The Fixed Shader

The generator only runs on Windows (`dx9sdk/utilities/fxc.exe` + D3DX), so the compiled `.vcs` is checked in for macOS/Linux users. Install by copying `game/hl2/*` over the Half-Life 2 game directory, or copy the file into any mounted search path, e.g.:

```text
<HL2 dir>/hl2/custom/flashlight-fix/shaders/fxc/worldtwotextureblend_ps20b.vcs
```

(`hl2/custom/*` is mounted automatically.) The engine loads `shaders/fxc/worldtwotextureblend_ps20b.vcs` at material load, so the file takes effect on next launch. The ps20 variant does not need regeneration: the shadow path is `PS_2_B`-only and the compiled ps_2_0 combos are byte-identical.
